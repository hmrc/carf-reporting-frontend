    ...
    SummaryListViewModel(
      rows = Seq(
        SummaryListRowViewModel(
          key = KeyViewModel(
            Text(messages(s"$messagePrefix.summary.messageRefId.label"))
          ),
        ...
        SummaryListRowViewModel(
          key = KeyViewModel(
            Text(messages(s"$messagePrefix.summary.result.label"))
          ),
          value = ValueViewModel(
            HtmlContent(GovukTag()(FileStatus.tagForFileStatus(fileStatus)))
          )
        )
      )
    )
