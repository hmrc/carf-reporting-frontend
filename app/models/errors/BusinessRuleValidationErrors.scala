...
case class BusinessRuleValidationErrors(
    fileError: Seq[FileError] = Seq.empty,
    recordError: Seq[RecordError] = Seq.empty
)

object BusinessRuleValidationErrors {
  implicit val format: OFormat[BusinessRuleValidationErrors] = Json.format[BusinessRuleValidationErrors]
}
