...
object DataErrorsStubData {
  ...
  val fewErrors: Seq[SchemaError] = Seq(
    ...
    SchemaError(
      10,
      Html(
        """<p class="govuk-!-margin-bottom-1">MessageRefId element must be from 26 to 100 characters. It must also match the file name and include the following in the order referenced:</p>
          ...
          |</ul>
          |<p class="govuk-!-margin-bottom-0">MessageRefId must also not include less than signs (<), greater than signs (>), colons (:), straight double quotes ("), apostrophes ('), ampersands (&amp;), forward slashes (/), backslashes (\), vertical bars (|), question marks (?) or asterisks (*).</p>
          |""".stripMargin
      )
    ),
    SchemaError(
      12,
      Html(
        """<p class="govuk-!-margin-bottom-1">ReportingPeriod value must:</p>
          |<ul class="govuk-list govuk-list--bullet">
          ...
          |</ul>
          |<p class="govuk-!-margin-bottom-0">For example, 2026-12-31.</p>
          |""".stripMargin
      )
    ),
    ...
    SchemaError(
      41,
      Html(
        """<p class="govuk-!-margin-bottom-1">DocRefId element must be from 28 to 164 characters and include the following in the order referenced:</p>
          |<ul class="govuk-list govuk-list--bullet">
          ...
          |</ul>
          |<p class="govuk-!-margin-bottom-0">For an OECD0 file, the DocRefId must match the previous submission.</p>
          |""".stripMargin
      )
    ),
